  ...
  return (
    ...
    >
      {item.type === "image" ? (
        ...
        <span className="whitespace-nowrap text-sm sm:text-base md:text-xl lg:text-3xl font-patua text-white text-center">
          {item.content}
        </span>
      )}
    </div>
    ...
    <div className={cn("w-full overflow-hidden bg-[#04041e] py-2 sm:py-4 md:py-6 lg:py-8 border-t-4 border-b-4 border-cuba-red", className)}>
      <div className="relative flex w-full">
        {[1, 2].map((setIndex) => (
          <div 
            key={`set-${setIndex}`}
            className="animate-scroll flex min-w-full items-center justify-start sm:justify-evenly space-x-4 sm:space-x-8 md:space-x-12 lg:space-x-16 px-2 sm:px-4 md:px-6 lg:px-8"
          >
            {items.map((item) => (
