  ...
  return (
    <div className={cn("w-full overflow-hidden bg-[#0A0F29] py-2 md:py-8", className)}>
      <div className="relative flex w-full">
        ...
        <div className="animate-scroll flex min-w-full items-center justify-between md:justify-evenly px-4 md:px-8">
          {items.map((item, idx) => (
            <div key={`first-${idx}`} className="flex items-center mx-2 md:mx-0">
              {item.type === "image" ? (
                <img
                  src={item.content}
                  alt="Country Takeover Logo"
                  className="h-10 w-10 md:h-24 md:w-24 rounded-full object-cover"
                />
              ) : (
                <span className="whitespace-nowrap text-base md:text-3xl font-bold text-white">
                  {item.content}
                </span>
              )}
            </div>
        ...
        <div className="animate-scroll flex min-w-full items-center justify-between md:justify-evenly px-4 md:px-8">
          {items.map((item, idx) => (
            <div key={`second-${idx}`} className="flex items-center mx-2 md:mx-0">
              {item.type === "image" ? (
                <img
                  src={item.content}
                  alt="Country Takeover Logo"
